...
func newTestPushProvider(serverURL string) *PushProvider {
	...
	return NewPushProvider(config)
}

func TestPushProvider_Send_SuccessWithValidJSON(t *testing.T) {
	...
	p := newTestPushProvider(server.URL)
	req := &provider.ProviderRequest{
		To:      "device-token-abc",
		Channel: "push",
		Content: "Hello via Push",
	}
	...
	assert.Equal(t, "push-12345", resp.MessageID)
...
func TestPushProvider_Send_SuccessWithEmptyMessageID_FallsBackToGenerated(t *testing.T) {
...
func TestPushProvider_Send_ConnectionError(t *testing.T) {
	p := newTestPushProvider("http://127.0.0.1:1")
	req := &provider.ProviderRequest{
		To:      "device-token-abc",
		Channel: "push",
		Content: "Hello via Push",
...
func TestPushProvider_Send_EmptyResponseBody_200(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer server.Close()
...
func TestPushProvider_ImplementsNotificationProvider(t *testing.T) {
	var _ provider.NotificationProvider = (*PushProvider)(nil)
}

func TestPushProvider_Send_VerifiesRequestHeaders(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
